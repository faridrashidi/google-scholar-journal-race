...
class Navigator(metaclass=Singleton):
                    ...
                    if not pm.has_proxy():
                        ...
                        if not self.got_403:
                            ...
                            if pm.proxy_mode not in (
                                ...
                                self.logger.info(
                                    "Will retry after %.2f seconds (with another"
                                    " session).",
                                    w,
                                )
                                time.sleep(w)
    ...
    def search_author_id(
        ...
        :type filled: bool, optional
        :param sortby: if the object is an author, select the order of the citations in
        the author page. Either by 'citedby' or 'year'. Defaults to 'citedby'.
        :type sortby: string
        :param publication_limit: Select the max number of publications you want you
        want to fill for the author. Defaults to no limit.
    ...
    def search_organization(self, url: str, fromauthor: bool) -> list:
        ...
        if no results are found and `fromuthor` is True, then use the first author
        from the search
